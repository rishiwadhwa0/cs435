...
#so... just do it twice!
`sudo ifconfig $net_name:manager up 10.0.0.10 2>/dev/null`;
`sudo ifconfig $net_name:manager up 10.0.0.10 2>/dev/null`;
...
while(my $curLine = <TOPOFILE>)
{
	...
	`sudo ifconfig $net_name:$nodes[0] up $addr0 2>/dev/null`;
	`sudo ifconfig $net_name:$nodes[0] up $addr0 2>/dev/null`;
	`sudo ifconfig $net_name:$nodes[1] up $addr1 2>/dev/null`;
	`sudo ifconfig $net_name:$nodes[1] up $addr1 2>/dev/null`;
